...
def odefunction(z, c, u_S):
    ...
    R_1 = k_1 / P_H2 ** 2.5 * (P_CH4 * P_H2O - (P_H2 ** 3 * P_CO) / K_1) / den ** 2
    R_2 = k_2 / P_H2 ** 3.5 * (P_CH4 * P_H2O ** 2 - (P_H2 ** 4 * P_CO2) / K_2) / den ** 2
    R_3 = k_3 / P_H2 * (P_CO * P_H2O - P_H2 * P_CO2 / K_3) / den ** 2
    
    rho_g = (1/(R * T)) * (M_CH4 * P_CH4 + M_H2O * P_H2O + M_H2 * P_H2 + M_CO * P_CO + M_CO2 * P_CO2) * 100     #Masse volumique de la phase gazeuse avec les p en Pa (kg/m^3)
